...
async function main() {
  ...
  const flags = await get('/config/feature-flags');
  checks.push({ name: 'GET /config/feature-flags', ok: flags.status === 200 });

  if (token) {
    const blocks = await get('/users/me/blocks', true);
    checks.push({
      name: 'GET /users/me/blocks (UGC moderation)',
      ok: blocks.status === 200 && Array.isArray(blocks.body),
    });
    const saved = await get('/sites/saved?limit=5', true);
    checks.push({
