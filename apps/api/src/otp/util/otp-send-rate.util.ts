...
export function tryOtpSendAllowed(
  existing: OtpSendState | null,
  nowMs = Date.now(),
): { sendCountInWindow: number; sendWindowStartedAt: Date } | null {
...
export function assertOtpSendAllowed(existing: OtpSendState | null, nowMs = Date.now()): {
  ...
  const allowed = tryOtpSendAllowed(existing, nowMs);
  if (allowed) {
    return allowed;
  }
