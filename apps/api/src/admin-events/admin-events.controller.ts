 ...
 *   any message as stale and reconnect (see repo root `docs/admin-realtime.md`).
...
export class AdminEventsController {
  constructor(
    ...
    private readonly cleanupEventsEventsService: CleanupEventsEventsService,
  ) {}
  ...
  stream(): Observable<NestMessageEvent> {
    ...
    const cleanupEvents = this.cleanupEventsEventsService.getEvents().pipe(
      map((event) => ({
        data: event as object,
        type: event.type,
      })),
