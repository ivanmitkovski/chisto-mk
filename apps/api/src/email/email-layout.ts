...
export function buildDetailCardHtml(rows: { label: string; value: string }[]): string {
  ...
  const rowsHtml = rows
    .map(
      (r) =>
        `<tr><td style="padding:8px 14px 8px 0;font-size:13px;font-weight:600;color:${EMAIL_BRAND.textMuted};vertical-align:top;white-space:nowrap;">${esc(r.label)}</td>` +
        `<td style="padding:8px 0;font-size:15px;line-height:1.45;color:${EMAIL_BRAND.textPrimary};">${esc(r.value)}</td></tr>`,
    )
    .join('');
  return (
