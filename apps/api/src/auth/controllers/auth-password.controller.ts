...
import { PasswordResetEmailVerifyDto } from '../dto/password-reset-email-verify.dto';
...
export class AuthPasswordController {
  ...
  @ApiOperation({ summary: 'Request password reset via SMS or email code' })
  @ApiOkResponse({
    description:
      'Generic success message. When an account exists, a 6-digit code is sent. devCode returned in development only.',
  })
  ...
  @Post('password-reset/email/verify-code')
  @Throttle({ default: { ttl: 60_000, limit: 10 } })
  @ApiOperation({ summary: 'Verify password-reset email code before setting a new password' })
  @ApiNoContentResponse({ description: 'Code is valid; OTP is not consumed until confirm' })
  @HttpCode(HttpStatus.NO_CONTENT)
  async verifyEmailPasswordResetCode(@Body() dto: PasswordResetEmailVerifyDto): Promise<void> {
    await this.passwordReset.verifyPasswordResetCodeByEmail(dto.email, dto.code);
  }
  ...
  confirmEmailPasswordReset(@Body() dto: PasswordResetEmailConfirmDto): Promise<{ message: string }> {
    return this.passwordReset.confirmEmailReset(dto.email, dto.code, dto.newPassword);
  }
