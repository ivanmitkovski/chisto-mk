...
import { TypesenseSitesSearchService } from '../search/typesense/typesense-sites-search.service';
import { MAP_SEARCH_KM_PER_DEGREE } from '../util/sites-map-search-geo-intent';
...
export class SitesMapSearchQueryService {
  ...
  constructor(
    ...
    @Optional() private readonly typesenseSearch?: TypesenseSitesSearchService,
  ) {}
  ...
  async executeSearch(
    ...
    if (this.typesenseSearch?.isEnabled()) {
      try {
        return await this.typesenseSearch.search(dto, viewerUserId);
      } catch (error) {
        this.logger.warn(
          `Typesense search failed, falling back to Postgres: ${String(error)}`,
        );
      }
    }

    return this.executePostgresSearch(dto, viewerUserId);
  }

  private async executePostgresSearch(
    dto: SiteMapSearchDto,
    viewerUserId?: string | null,
  ): Promise<RawSearchRow[]> {
    ...
    const visibilityClause = siteVisibilitySql({
