...
enum EngagementOutboxKind { upvote, removeUpvote, save, unsave }
...
class EngagementOutboxStore {
  ...
  Future<void> enqueue(EngagementOutboxEntry entry) => _serialized(() async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    final List<EngagementOutboxEntry> current = _decode(
      prefs.getString(_prefKey),
    );
    current.removeWhere(
      (EngagementOutboxEntry e) =>
          e.siteId == entry.siteId && e.kind == entry.kind,
    );
    current.add(entry);
  ...
  Future<void> recordRetryableFlushFailure(String id, {int maxFailures = 5}) =>
      _serialized(() async {
        if (id.isEmpty) return;
        final SharedPreferences prefs = await SharedPreferences.getInstance();
        final List<EngagementOutboxEntry> current = _decode(
        ...
        final int index = current.indexWhere(
          (EngagementOutboxEntry e) => e.id == id,
        );
        if (index < 0) return;
        ...
        await prefs.setString(
          _prefKey,
          jsonEncode(
            current.map((EngagementOutboxEntry x) => x.toJson()).toList(),
          ),
        );
      });
  ...
  static Future<void> enqueueUpvoteIntent({
    ...
    final EngagementOutboxEntry entry = EngagementOutboxEntry(
      ...
      kind: wantUpvoted
          ? EngagementOutboxKind.upvote
          : EngagementOutboxKind.removeUpvote,
      siteId: siteId,
