...
class SitesLocalCache {
  ...
  Future<void> persistFeedSnapshot({
    ...
    final Map<String, dynamic> feeds =
        safeAsStringKeyedMap(decoded['feeds']) ?? <String, dynamic>{};
    final Map<String, dynamic> scope =
        safeAsStringKeyedMap(feeds[scopeKey]) ?? <String, dynamic>{};
    final List<dynamic> existingPages =
        safeAsList(scope['pages']) ?? <dynamic>[];
    ...
    if (feeds.length > maxPersistedFeeds) {
      final List<MapEntry<String, dynamic>> entries = feeds.entries.toList()
        ..sort((MapEntry<String, dynamic> a, MapEntry<String, dynamic> b) {
          final int aMs =
              (safeAsStringKeyedMap(a.value)?['updatedAtMs'] as num?)
                  ?.toInt() ??
              0;
          final int bMs =
              (safeAsStringKeyedMap(b.value)?['updatedAtMs'] as num?)
                  ?.toInt() ??
              0;
  ...
  loadFeedPage({
    ...
    final Map<String, dynamic> feeds =
        safeAsStringKeyedMap(decoded['feeds']) ?? const <String, dynamic>{};
    final Map<String, dynamic>? scope = safeAsStringKeyedMap(feeds[scopeKey]);
    if (scope == null) return null;
    final List<dynamic> pages = safeAsList(scope['pages']) ?? const <dynamic>[];
    ...
    final int storedPage = ((selected['page'] as num?)?.toInt() ?? page).clamp(
      1,
      999999,
    );
