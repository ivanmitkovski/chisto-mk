...
import 'package:feature_home/src/data/map_regions/map_region_geometry.dart';
...
class MapBoundariesRepository {
  ...
  Future<void> _load() async {
    try {
      ...
      final Map<String, dynamic>? json = safeJsonDecodeMap(raw);
      if (json == null) {
        return;
      }
      final List<dynamic> features =
          safeAsList(json['features']) ?? const <dynamic>[];
      ...
      for (final dynamic entry in features) {
        final Map<String, dynamic>? feature = safeAsStringKeyedMap(entry);
        if (feature == null) {
          continue;
        }
        final Map<String, dynamic>? props = safeAsStringKeyedMap(
          feature['properties'],
        );
        ...
        final Map<String, dynamic>? geometry = safeAsStringKeyedMap(
          feature['geometry'],
        );
        final List<MapRegionPolygonGeometry> polygons = _parseGeometry(
          geometry,
        );
        if (polygons.isEmpty) {
          continue;
  ...
  String? _extractMapId(Map<String, dynamic>? props) {
    ...
    final String? shapeName =
        props?['mapName'] as String? ?? props?['shapeName'] as String?;
  ...
  List<MapRegionPolygonGeometry> _parseGeometry(
    ...
    if (type == 'Polygon') {
      final MapRegionPolygonGeometry? polygon = _parsePolygonCoordinates(
        coordinates,
      );
      return polygon == null
    ...
    if (type == 'MultiPolygon') {
      final List<dynamic>? all = safeAsList(coordinates);
      if (all == null) {
        return const <MapRegionPolygonGeometry>[];
      }
      return all
          .map<MapRegionPolygonGeometry?>(_parsePolygonCoordinates)
  ...
  MapRegionPolygonGeometry? _parsePolygonCoordinates(dynamic coordinates) {
    final List<dynamic>? ringsRaw = safeAsList(coordinates);
  ...
  List<LatLng> _parseRing(dynamic ringRaw) {
    final List<dynamic>? pointsRaw = safeAsList(ringRaw);
    ...
    for (final dynamic pointRaw in pointsRaw) {
      final List<dynamic>? coords = safeAsList(pointRaw);
  ...
  List<LatLng> _normalizeRing(List<LatLng> points) {
    ...
    for (final LatLng p in points) {
      ...
      final bool same =
          prev.latitude == p.latitude && prev.longitude == p.longitude;
      if (!same) {
        deduped.add(p);
      }
    }
    ...
    final Set<String> unique = deduped
        .map(
          (LatLng p) =>
              '${p.latitude.toStringAsFixed(8)},${p.longitude.toStringAsFixed(8)}',
        )
        .toSet();
    if (unique.length < 3) {
      return const <LatLng>[];
  ...
  List<LatLng> _simplifyRing(List<LatLng> points) {
    ...
    final List<LatLng> simplified = _douglasPeucker(
      open,
      _simplifyToleranceDegrees,
    );
  ...
  List<LatLng> _douglasPeucker(List<LatLng> points, double epsilon) {
    ...
    final List<LatLng> left = _douglasPeucker(
