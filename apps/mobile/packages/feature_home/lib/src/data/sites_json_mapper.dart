...
class SitesJsonMapper {
  ...
  PollutionSite siteDetailFromJson(Map<String, dynamic> json) {
    ...
    final List<dynamic> reportsJson =
        safeAsList(root['reports']) ?? <dynamic>[];
    final List<Map<String, dynamic>> reports = sitesApiMapListOfObjects(
      reportsJson,
    );
    final List<dynamic> eventsJson = safeAsList(root['events']) ?? <dynamic>[];
    ...
    List<String> heroMediaUrls = sitesApiStringListFromJsonField(
      root['heroMediaUrls'] ?? root['hero_media_urls'],
    );
    ...
    if (reports.isNotEmpty) {
      ...
      final List<dynamic> firstMediaList = sitesApiReportMediaUrlsList(
        earliest,
      );
      final List<String> firstReportImageUrls = firstMediaList
          ...
          .toList();
      if (heroMediaUrls.isEmpty) {
        heroMediaUrls = firstReportImageUrls;
      }
    ...
    final Map<String, dynamic>? canonicalReportJson = reports.isNotEmpty
        ? sitesApiEarliestReport(reports)
        : null;
    final String canonicalTitle = normalizeFeedTitle(
      canonicalReportJson?['title'] as String? ?? '',
    );
    final String canonicalDesc =
        canonicalReportJson?['description'] as String? ?? '';
    final String title = desc.isNotEmpty
        ? desc
        : (canonicalTitle.trim().isNotEmpty
              ? canonicalTitle.trim()
              : (canonicalDesc.isNotEmpty ? canonicalDesc : 'Site'));
    ...
    return PollutionSite(
      ...
      description: desc.isNotEmpty
          ? desc
          : (canonicalTitle.trim().isNotEmpty && canonicalDesc.isNotEmpty
                ? canonicalDesc
                : (canonicalTitle.trim().isNotEmpty
                      ? canonicalTitle
                      : canonicalDesc)),
      ...
      latestReporterName: () {
        ...
        final String name = sitesApiPublicReporterDisplayName(reporterJson);
        return name == kSitesApiAnonymousCoReporterName ? null : name;
      }(),
      latestReporterAvatarUrl: () {
        ...
        final Object? av =
            reporterJson?['avatarUrl'] ?? reporterJson?['avatar_url'];
  ...
  SiteMapSearchResponse siteMapSearchResponseFromJson(
    Map<String, dynamic> json,
  ) {
    final List<dynamic> itemsRaw = safeAsList(json['items']) ?? <dynamic>[];
    final List<PollutionSite> items = itemsRaw
        .whereType<Map<String, dynamic>>()
        .map(mapSearchItemToPollutionSite)
        .toList();
    final List<dynamic> sugRaw = safeAsList(json['suggestions']) ?? <dynamic>[];
  ...
  PollutionSite mapSearchItemToPollutionSite(Map<String, dynamic> json) {
    ...
    final double? lat = (json['latitude'] as num?)?.toDouble();
    final double? lng = (json['longitude'] as num?)?.toDouble();
    final List<dynamic>? mediaRaw = safeAsList(json['latestReportMediaUrls']);
