  ...
  MapViewportQuery shiftedBy({
    ...
    return MapViewportQuery(
      ...
      minLongitude: minLongitude != null
          ? minLongitude! + deltaLngDegrees
          : null,
      maxLongitude: maxLongitude != null
          ? maxLongitude! + deltaLngDegrees
          : null,
    );
  }
...
class MapSyncCoordinator extends ChangeNotifier {
  ...
  // ignore: use_setters_to_change_properties, coordinator API used by tests
  void updateViewport(MapViewportQuery query) {
    _currentQuery = query;
  }

  // ignore: avoid_positional_boolean_parameters, lifecycle API used by tests
  ...
  Future<void> _performFullSync({bool allowEnvelopeSkip = true}) async {
    ...
    if (_shouldSkipFetchForEnvelope(
      query,
      allowEnvelopeSkip: allowEnvelopeSkip,
    )) {
