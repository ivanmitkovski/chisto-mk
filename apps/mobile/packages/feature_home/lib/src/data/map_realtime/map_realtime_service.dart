    ...
    try {
      ...
      if (res.statusCode == 401) {
        try {
          ...
          AppLog.warn(
            '[map:sse] drain after 401 failed',
            error: e,
            stackTrace: st,
            category: 'map',
          );
      ...
      if (res.statusCode < 200 || res.statusCode >= 300) {
        try {
          ...
          AppLog.warn(
            '[map:sse] drain after HTTP error failed',
            error: e,
            stackTrace: st,
            category: 'map',
          );
  ...
  void _closeActiveConnection({bool disposeOnly = false}) {
    ...
    if (active != null) {
      ...
      if (_ownsHttpClient) {
        try {
          ...
          AppLog.warn(
            '[map:sse] active client close failed',
            error: e,
            stackTrace: st,
            category: 'map',
          );
    ...
    if (disposeOnly && !_ownsHttpClient && injected != null) {
      try {
        ...
        AppLog.warn(
          '[map:sse] injected client close failed',
          error: e,
          stackTrace: st,
          category: 'map',
        );
