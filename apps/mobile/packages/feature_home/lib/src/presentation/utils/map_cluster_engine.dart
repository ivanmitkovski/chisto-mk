...
List<ClusterBucket> buildMapClusterBuckets({
  ...
  assert(
    zoom.isFinite && cameraCenterLat.isFinite && cameraCenterLng.isFinite,
  ...
  final bool useGrid = ordered.length > _kGridClusterMinSites && threshold > 0;
...
List<Map<String, Object>> _clusterPointsIsolate(Map<String, Object> input) {
  ...
  final String? selectedSiteId = (input['selectedSiteId']! as String).isEmpty
      ? null
      : input['selectedSiteId']! as String;
  ...
  final Map<String, Set<int>> cellToBuckets = <String, Set<int>>{};
  final bool useGrid =
      rawPoints.length > _kGridClusterMinSites && threshold > 0;
...
Future<List<ClusterBucket>> buildMapClusterBucketsAdaptive({
  ...
  for (final Map<String, Object> raw in rawBuckets) {
    ...
    buckets.add(
      ClusterBucket(
        center: LatLng(
          raw['centerLat']! as double,
          raw['centerLng']! as double,
        ),
        sites: bucketSites,
