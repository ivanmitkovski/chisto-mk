...
class _MapActionsMenuState extends State<MapActionsMenu>
  ...
  static const double _staggerStep = 0.11;
  static const double _staggerSpan = 0.45;

  static Interval _actionInterval(int index) {
    final double start = index * _staggerStep;
    final double end = (start + _staggerSpan).clamp(0.0, 1.0);
  ...
  List<Animation<double>> _buildItemAnimations(AnimationController controller) {
    return List<Animation<double>>.generate(
      _maxActions,
      (int i) => CurvedAnimation(parent: controller, curve: _actionInterval(i)),
    );
  }
  ...
  Widget _buildAction(BuildContext context, AppLocalizations l10n, int index) {
    switch (index) {
      case 0:
        return AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: _heatmapRingActive
                ? AppShadows.mapHeatmapRingActive()
                : null,
          ),
          child: _frostedButton(
            onTap: () {
        ...
        return AnimatedContainer(
          ...
          child: _frostedButton(
            ...
            iconChild: widget.isLocating
                ? Padding(
                    padding: const EdgeInsets.all(AppSpacing.radius10),
...
class MapFilterButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final String counterText = isEmpty
        ? (hasFilterActive
              ? l10n.mapFilterCountNoMatch
              : l10n.mapFilterCountNoSites)
        : '$visibleCount';
    final String semanticStatus = isEmpty
        ? (hasFilterActive
              ? l10n.mapFilterButtonSemanticNoMatch
              : l10n.mapFilterButtonSemanticNoSites)
        : l10n.mapFilterButtonSemanticSitesMatch(visibleCount);

    final Color fg = useDarkTiles
        ? AppColors.textOnDark
        : AppColors.textPrimary;
    final Color iconFg = useDarkTiles
            ...
            child: BackdropFilter(
              ...
              child: AnimatedContainer(
                ...
                constraints: const BoxConstraints(
                  minHeight: _mapControlTapTargetSize,
                ),
                decoration: BoxDecoration(
                  color: hasFilterActive
                      ? AppColors.primary.withValues(
                          alpha: useDarkTiles ? 0.2 : 0.12,
                        )
                      : useDarkTiles
                  ...
                  children: <Widget>[
                    ...
                    AnimatedSwitcher(
                      ...
                      child: Text(
                        ...
                        style: AppTypographySurfaces.homeMapFilterCounter(
