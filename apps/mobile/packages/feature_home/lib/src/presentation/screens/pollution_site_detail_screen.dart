...
class _PollutionSiteDetailScreenState
    extends ConsumerState<PollutionSiteDetailScreen>
  ...
  Future<void> _refreshSiteDetails() async {
    try {
      final PollutionSite? refreshed = await ref
          .read(sitesRepositoryProvider)
          .getSiteById(_site.id);
      if (!mounted || refreshed == null) return;
      final double resolvedDistanceKm = refreshed.distanceKm >= 0
      ...
      if (refreshed.latitude != null && refreshed.longitude != null) {
        _siteCoordinates[refreshed.id] = LatLng(
          refreshed.latitude!,
          refreshed.longitude!,
        );
      }
  ...
  void _applyEngagementToSite() {
    final SiteEngagementState eng = ref.read(
      siteEngagementNotifierProvider(_site.id),
    );
    setState(() {
  ...
  Widget build(BuildContext context) {
    final SiteEngagementState engagement = ref.watch(
      siteEngagementNotifierProvider(_site.id),
    );
