...
class MapSiteCorpusNotifier extends Notifier<MapSiteCorpusState> {
  ...
  Future<void> _tryRestoreFromDisk() async {
    ...
    final List<PollutionSite> sites = loaded.sites
        ...
        .toList();
    state = MapSiteCorpusState(sites: sites, lastFetchedAt: DateTime.now());
  }
    ...
    try {
      ...
      if (e.code == 'TOO_MANY_REQUESTS') {
        state = state.copyWith(
          isRefreshing: false,
          lastError: e,
          rateLimitedUntil: DateTime.now().add(kMapSiteCorpusRateLimitBackoff),
        );
        return;
      }
