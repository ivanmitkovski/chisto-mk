...
class SiteEngagementOutcome {
  ...
  const SiteEngagementOutcome.success()
    : this._(SiteEngagementOutcomeKind.success);
...
class SiteEngagementState {
  ...
  factory SiteEngagementState.fromSite(PollutionSite site) =>
      SiteEngagementState(
        siteId: site.id,
...
final siteEngagementNotifierProvider = NotifierProvider.autoDispose
    .family<SiteEngagementNotifier, SiteEngagementState, String>(
      SiteEngagementNotifier.new,
    );

class SiteEngagementNotifier
    extends AutoDisposeFamilyNotifier<SiteEngagementState, String> {
  String get _siteId => arg;

  @override
  SiteEngagementState build(String siteId) =>
      SiteEngagementState.initial(siteId);
  ...
  Future<EngagementSnapshot> _upvoteCall(bool nextUpvoted) {
    return nextUpvoted
        ? _repo.upvoteSite(_siteId)
        : _repo.removeSiteUpvote(_siteId);
  }
  ...
  Future<SiteEngagementOutcome> toggleUpvote() async {
    ...
    state = state.copyWith(
      ...
      upvoteCount: (state.upvoteCount + (nextUpvoted ? 1 : -1)).clamp(
        0,
        999999,
      ),
    ...
    try {
      final EngagementSnapshot snap = await _withSingleRetryOnRetryable(
        () => _upvoteCall(nextUpvoted),
      );
      state = state.applySnapshot(snap);
      return const SiteEngagementOutcome.success();
    } on AppError catch (e) {
      state = previous;
      final bool queueableOffline =
          e.retryable &&
