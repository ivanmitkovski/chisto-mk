...
String formatPhoneForDisplay(String? value) {
  ...
  final String local = digits.length >= 8
      ? digits.substring(digits.length - 8)
      : digits;
  if (local.length <= 2) return local;
  if (local.length <= 5) {
    return '+389 ${local.substring(0, 2)} ${local.substring(2)}';
  }
...
///
/// Alias for live phone inputs: [formatMacedonianLocalPhone].
...
/// Formats up to 8 national digits while the user types (auth sign-up / sign-in).
String formatMacedonianLocalPhone(String digits) {
  if (digits.isEmpty) return '';
  final String clipped = digits.length > 8 ? digits.substring(0, 8) : digits;
  return formatPhoneNationalPart(clipped);
}
