...
extension EventChatStreamMixin on _EventChatScreenState {
  ...
  int _indexOfPendingOwnMediaMessage(EventChatMessage m) {
    ...
    final int serverSum = m.attachments.fold<int>(
      0,
      (int a, EventChatAttachment c) => a + c.sizeBytes,
    );
    for (final int i in candidates) {
      final int localSum = _messages[i].attachments.fold<int>(
        0,
        (int a, EventChatAttachment c) => a + c.sizeBytes,
      );
  ...
  int _indexOfPendingOwnVoiceNote(EventChatMessage m) {
    final EventChatAttachment? serverA = m.attachments.isEmpty
        ? null
        : m.attachments.first;
    if (serverA == null) {
      return -1;
  ...
  void _onStreamEvent(EventChatStreamEvent e) {
    ...
    if (e is EventChatStreamMessageCreated) {
      ...
      final int idx = _messages.indexWhere(
        (EventChatMessage x) => x.id == e.messageId,
      );
      if (idx >= 0) {
        rebuildState(() {
      ...
      final int i = _readCursors.indexWhere(
        (EventChatReadCursor c) => c.userId == e.userId,
      );
  ...
  void _onComposerTextChanged(String t) {
    ...
    _typingDebounce = Timer(const Duration(milliseconds: 300), () {
      ...
      if (!_typingOutboundSent) {
        ...
        unawaited(_repo.setTyping(widget.eventId, typing: true));
      }
    });
