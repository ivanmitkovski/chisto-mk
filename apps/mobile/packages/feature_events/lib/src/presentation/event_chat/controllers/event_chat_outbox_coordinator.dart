...
extension EventChatOutboxMixin on _EventChatScreenState {
  ...
  Future<void> _flushOfflineQueue() async {
    ...
    for (int n = 0; n < maxIterationsPerRun && mounted; n++) {
      final ChatOutboxEntry? q = await ChatOutboxStore.shared.peekNext(
        widget.eventId,
      );
      if (q == null) {
        break;
      ...
      if (res.kind == ChatOutboxFlushKind.sent) {
        ...
        rebuildState(() {
          final int i = _messages.indexWhere(
            (EventChatMessage m) => m.id == q.tempId,
          );
          if (i >= 0) {
            _messages[i] = saved;
          }
  ...
  Future<void> _reconcileChatOutboxAfterInitialLoad() async {
    ...
    final List<ChatOutboxEntry> rows = await ChatOutboxStore.shared
        .listPendingAndFailed(widget.eventId);
    ...
    for (final ChatOutboxEntry row in rows) {
      final bool hasBubble = _messages.any(
        (EventChatMessage m) =>
            m.id == row.tempId || m.clientMessageId == row.clientMessageId,
      );
      if (hasBubble) {
        continue;
      }
