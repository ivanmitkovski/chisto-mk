...
extension EventChatSendMediaMixin on _EventChatScreenState {
  ...
  Future<void> _finalizeVoiceSend(
    String tempId,
    XFile file,
    ...
    try {
      ...
      final List<EventChatAttachment> uploaded = await _repo.uploadAttachments(
        widget.eventId,
        <UploadableFile>[
          UploadableFile(bytes: bytes, fileName: file.name, mimeType: mime),
        ],
      ...
      final int pendingRow = _messages.indexWhere(
        (EventChatMessage m) => m.id == tempId,
      );
      if (pendingRow >= 0) {
      ...
      chatDiagLog('voice_send_ok', <String, Object?>{
        'eventId': widget.eventId,
      });
      ...
      if (e.code == 'CANCELLED') {
        ...
        chatDiagLog('voice_upload_cancelled', <String, Object?>{
          'eventId': widget.eventId,
        });
        return;
      }
  ...
  EventChatMessageType _messageTypeForAttachmentBatch(List<String> mimesLower) {
    ...
    final bool allImage = mimesLower.every(
      (String m) => m.startsWith('image/'),
    );
    if (allImage) {
  ...
  Future<void> _finalizeAttachmentSend(
    ...
    try {
      ...
      chatDiagLog('attachment_send_ok', <String, Object?>{
        'eventId': widget.eventId,
      });
      unawaited(_markReadBestEffort());
    } on AppError catch (e) {
      ...
      if (e.code == 'CANCELLED') {
        ...
        chatDiagLog('attachment_upload_cancelled', <String, Object?>{
          'eventId': widget.eventId,
        });
        return;
      }
