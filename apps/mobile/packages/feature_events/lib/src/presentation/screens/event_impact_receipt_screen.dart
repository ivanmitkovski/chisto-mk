...
class EventImpactReceiptScreen extends ConsumerStatefulWidget {
  const EventImpactReceiptScreen({super.key, required this.eventId});

  final String eventId;

  @override
  ConsumerState<EventImpactReceiptScreen> createState() =>
      _EventImpactReceiptScreenState();
}

class _EventImpactReceiptScreenState
    extends ConsumerState<EventImpactReceiptScreen> {
  bool _loading = true;
  ...
  Future<void> _fetch() async {
    ...
    try {
      final EventImpactReceipt receipt = await readEventsRepository()
          .fetchImpactReceipt(widget.eventId);
      if (!mounted) {
        return;
      }
      setState(() {
  ...
  String _completenessLabel(
    AppLocalizations l10n,
    EventImpactReceiptCompleteness c,
  ) {
    switch (c) {
  ...
  Future<void> _copyLink(EventImpactReceipt receipt) async {
    final String url = eventSharePageUrl(
                    ...
                    child: Column(
                      ...
                      children: <Widget>[
                        ...
                        Text(
                          DateFormat.yMMMd(
                            l10n.localeName,
                          ).add_jm().format(r.scheduledAt.toLocal()),
