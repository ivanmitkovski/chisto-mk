...
DateTime ceilToMinuteGrid(
  DateTime dt, {
  int stepMinutes = kEventScheduleTimeGridMinutes,
}) {
  if (stepMinutes <= 0) {
    return dt;
  }
...
ScheduleValidationIssue? validateInProgressEditSchedule({
  ...
  if (endInstant.isBefore(earliestEnd) ||
      endInstant.isAtSameMomentAs(earliestEnd)) {
...
({EventTime start, EventTime end}) defaultStartEndForDate({
  ...
  if (d.isBefore(today)) {
    ...
    final DateTime startInstant = DateTime(
      today.year,
      today.month,
      today.day,
      s.hour,
...
({EventTime start, EventTime end}) clampCreateOrUpcomingSchedule({
  ...
  if (issue == ScheduleValidationIssue.endNotAfterStart ||
    ...
    final DateTime dayEnd = eventScheduleLocalDayEnd(d);
    DateTime bumped = ceilToMinuteGrid(
      si.add(Duration(hours: defaultDurationHours)),
    );
...
({EventTime start, EventTime end}) clampInProgressEditSchedule({
  ...
  final DateTime d = DateUtils.dateOnly(dateOnly);
  final EventTime en = clampEndTimeToEventDay(
    dateOnly: d,
    end: end,
    start: start,
  );
  ...
  if (!endCandidate.isAfter(earliestEnd)) {
    endCandidate = ceilToMinuteGrid(
      earliestEnd.add(const Duration(minutes: 1)),
    );
    if (!endCandidate.isAfter(startInstant) &&
        !endCandidate.isAfter(earliestEnd)) {
      endCandidate = ceilToMinuteGrid(
...
DateTime _pickerShellFromInstant(DateTime dt) =>
    _pickerShell(dt.hour, dt.minute);
