...
bool _sameChatGroup(List<EventChatMessage> messages, int a, int b) {
  ...
  if (ma.messageType == EventChatMessageType.system ||
      mb.messageType == EventChatMessageType.system) {
    return false;
  }
...
bool _isLastInGroup(List<EventChatMessage> messages, int i) {
  ...
  while (n < messages.length &&
      messages[n].messageType == EventChatMessageType.system) {
    n++;
  }
...
List<EventChatMessageGroupingInfo> computeEventChatMessageGrouping(
  ...
  return List<EventChatMessageGroupingInfo>.generate(messages.length, (int i) {
    ...
    final bool date =
        i == 0 || !_sameDay(messages[i].createdAt, messages[i - 1].createdAt);
    return EventChatMessageGroupingInfo(
      isFirst: first,
      isLast: last,
      showDate: date,
    );
...
double eventChatBubbleGapBelow({
  ...
  final bool newerOpensDay =
      i + 1 < grouping.length && grouping[i + 1].showDate;
  if (newerOpensDay) {
