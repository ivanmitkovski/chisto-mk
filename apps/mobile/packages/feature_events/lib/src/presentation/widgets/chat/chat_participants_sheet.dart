...
List<EventChatParticipantPreview> sortedChatParticipants(
  ...
  final List<EventChatParticipantPreview> copy =
      List<EventChatParticipantPreview>.from(raw);
...
class _ChatParticipantsSheetBody extends StatefulWidget {
  ...
  @override
  State<_ChatParticipantsSheetBody> createState() =>
      _ChatParticipantsSheetBodyState();
}

class _ChatParticipantsSheetBodyState
    extends State<_ChatParticipantsSheetBody> {
  ...
  void initState() {
    super.initState();
    _participants = sortedChatParticipants(
  ...
  Future<void> _refresh({required bool userPulled}) async {
    try {
      final EventChatParticipantsResult p = await widget.repo.fetchParticipants(
        widget.eventId,
      );
      if (!mounted) {
        return;
      ...
      if (mounted && (userPulled || !hadList)) {
        AppSnack.show(
          context,
          message: context.l10n.eventChatParticipantsLoadError,
        );
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return AppSheetScaffold(
      title: context.l10n.eventChatParticipantsSheetTitle,
      subtitle: _count > 0
          ? context.l10n.eventChatParticipantsCount(_count)
          : null,
      useModalRouteShape: true,
      titleTextStyle: AppTypography.eventsCalendarMonthTitle(textTheme),
      subtitleTextStyle: AppTypography.eventsChatTimestamp(textTheme),
                ...
                child: _participants.isEmpty
                    ...
                    : ListView.separated(
                        ...
                        itemBuilder: (BuildContext c, int i) {
                          final EventChatParticipantPreview p =
                              _participants[i];
                                  ...
                                  child: Column(
                                    ...
                                    children: <Widget>[
                                      Text(
                                        ...
                                        style:
                                            AppTypography.eventsFormLeadHeading(
                                              textTheme,
                                            ),
                                      ),
                                      if (isMe)
                                        Text(
                                          context
                                              .l10n
                                              .eventChatParticipantsYouBadge,
                                          style:
                                              AppTypography.eventsSheetTextLink(
                                                textTheme,
                                              ).copyWith(
