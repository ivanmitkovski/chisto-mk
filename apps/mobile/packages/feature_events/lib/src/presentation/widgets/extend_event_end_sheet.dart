...
class _ExtendEventEndSheetState extends State<ExtendEventEndSheet> {
  ...
  void _scheduleConflictPreviewDebounced() {
    ...
    _scheduleConflictTimer = Timer(const Duration(milliseconds: 480), () {
      ...
      unawaited(() async {
        try {
          final EventScheduleConflictPreview preview = await widget
              .eventsRepository
    ...
    if (issue != null) {
      final String message = switch (issue) {
        ...
        ScheduleValidationIssue.endTooSoon =>
          context.l10n.eventsExtendEndTooSoon,
      };
    ...
    return ReportSheetScaffold(
      ...
      subtitle: context.l10n.eventsExtendEndSheetSubtitle(
        MaterialLocalizations.of(
          context,
        ).formatTimeOfDay(TimeOfDay.fromDateTime(_event.endDateTime)),
      ),
      fitToContent: true,
        ...
        children: <Widget>[
          Text(
            context.l10n.eventsExtendEndCurrentChoice(
              MaterialLocalizations.of(
                context,
              ).formatTimeOfDay(TimeOfDay.fromDateTime(_proposedEndLocal)),
            ),
            ...
            children: <Widget>[
              ActionChip(
                ...
                onPressed: _submitting
                    ? null
                    : () => _bumpEnd(const Duration(minutes: 15)),
              ),
              ActionChip(
                label: Text(context.l10n.eventsExtendEndPlus30),
                onPressed: _submitting
                    ? null
                    : () => _bumpEnd(const Duration(minutes: 30)),
              ),
              ActionChip(
                label: Text(context.l10n.eventsExtendEndPlus60),
                onPressed: _submitting
                    ? null
                    : () => _bumpEnd(const Duration(hours: 1)),
              ),
            ],
          ),
