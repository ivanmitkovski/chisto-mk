...
class ChatLocationPickerSheet extends StatefulWidget {
  ...
  @override
  State<ChatLocationPickerSheet> createState() =>
      _ChatLocationPickerSheetState();
}
...
class _ChatLocationPickerSheetState extends State<ChatLocationPickerSheet>
  ...
  Future<void> _goToCurrentLocation() async {
    ...
    try {
      ...
      if (!mounted) return;
      unawaited(_animatedMapController.animateTo(dest: loc, zoom: 16));
      setState(() {
        _center = loc;
        _locating = false;
      });
      unawaited(_reverseGeocode(loc));
    } on Object catch (_) {
      if (mounted) setState(() => _locating = false);
    }
  }
  ...
  void _onMapPositionChanged(dynamic position, bool hasGesture) {
    if (hasGesture) {
      final LatLng center = (position as MapCamera).center;
      setState(() {
        _center = center;
        _label = null;
      });
