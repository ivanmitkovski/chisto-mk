...
class _EventsCalendarViewState extends State<EventsCalendarView> {
  ...
  EcoEvent _earliestByStart(List<EcoEvent> events) {
    ...
    copy.sort(
      (EcoEvent a, EcoEvent b) => a.startDateTime.compareTo(b.startDateTime),
    );
    return copy.first;
  }
  ...
  Set<int> _getEventDayKeys() {
    if (_eventDayKeys != null &&
        identical(_eventDayKeysSource, widget.events)) {
  ...
  List<EcoEvent> get _eventsForSelectedDate {
    ...
    return widget.events.where((EcoEvent e) {
      final DateTime d = e.date;
      return d.year == _selectedDate!.year &&
          d.month == _selectedDate!.month &&
          d.day == _selectedDate!.day;
    }).toList()..sort((EcoEvent a, EcoEvent b) => a.date.compareTo(b.date));
  }

  List<DateTime> _buildDays() {
    final DateTime first = DateTime(_focusedMonth.year, _focusedMonth.month, 1);
  ...
  void _recomputeIncompleteHint() {
    ...
    final bool hint =
        widget.hasMorePages && !_monthHasAnyLoadedEvent(_focusedMonth);
