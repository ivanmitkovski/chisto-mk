...
class CheckInQrPayload {
  ...
  static CheckInQrPayload? tryParse(String? raw) {
    ...
    if (raw.startsWith(v2Prefix)) {
      ...
      try {
        final List<int> jsonBytes = base64Url.decode(_paddedBase64Url(bodyB64));
...
class CheckInSubmissionResult {
  ...
  @override
  int get hashCode =>
      Object.hash(status, checkedInAt, pointsAwarded, pendingId);
}

/// Result of organizer manual check-in for a joined volunteer.
class ManualCheckInResult {
  const ManualCheckInResult({required this.recorded, this.pointsAwarded = 0});
...
class CheckedInAttendee {
  ...
  @override
  bool operator ==(Object other) =>
      identical(this, other) || other is CheckedInAttendee && id == other.id;
