import 'package:feature_events/src/domain/discovery_skopje_week.dart';
...
class EcoEventSearchParams {
  ...
  String get offlineListCacheSuffix {
    final List<String> categoryKeys =
        categories.map((EcoEventCategory c) => c.key).toList()..sort();
    final List<String> statusKeys =
        statuses.map((EcoEventStatus s) => s.apiKey).toList()..sort();
    final String q = (query ?? '').trim();
    return 'f${Object.hash(q, categoryKeys.join(','), statusKeys.join(','), dateFrom?.millisecondsSinceEpoch, dateTo?.millisecondsSinceEpoch).toUnsigned(32).toRadixString(16)}';
  }
  ...
  @override
  int get hashCode =>
      Object.hash(query, categories, statuses, dateFrom, dateTo);
