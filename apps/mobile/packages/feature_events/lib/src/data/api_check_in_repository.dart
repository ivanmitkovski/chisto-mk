...
class ApiCheckInRepository extends ChangeNotifier implements CheckInRepository {
  ...
  @override
  Duration get payloadTtl => const Duration(seconds: _qrTtlSec - 10);
    ...
    try {
      ...
      for (;;) {
        ...
        final ApiResponse response = await _client.get(
          '/events/$id/check-in/attendees$q',
        );
        final Map<String, dynamic>? json = response.json;
        final List<dynamic>? raw = safeAsList(json?['data']);
        if (raw == null) {
          break;
        }
        merged.addAll(
  ...
  Future<CheckInSubmissionResult> submitScan({
    ...
    try {
      ...
      await _enqueueOfflineRedeem(expectedEventId, rawPayload);
      return const CheckInSubmissionResult(
      ...
      _revertOptimisticCheckInIfNeeded(expectedEventId);
      return CheckInSubmissionResult(
  ...
  void _revertOptimisticCheckInIfNeeded(String eventId) {
    final EcoEvent? current = _events.findById(eventId);
    if (current == null ||
        current.attendeeCheckInStatus != AttendeeCheckInStatus.checkedIn) {
      return;
    }
    _events.setAttendeeCheckInStatus(
      eventId: eventId,
  ...
  Future<void> _enqueueOfflineRedeem(String eventId, String rawPayload) async {
  ...
  Future<void> resolvePendingCheckIn({
    ...
    await _client.post(
      ...
      body: <String, dynamic>{'action': approve ? 'approve' : 'reject'},
    );
    if (approve) {
      try {
        await refreshAttendees(eventId);
