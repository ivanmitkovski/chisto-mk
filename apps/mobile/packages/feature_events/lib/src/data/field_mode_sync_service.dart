...
FieldModeBatchBuildResult buildFieldBatchFromQueueRows(
...
class FieldModeSyncService {
  FieldModeSyncService({required ApiClient client, FieldModeQueue? queue})
    : _client = client,
  ...
  Future<FieldModeSyncResult> syncPendingRows() async {
    ...
    try {
      ...
      if (json == null) {
        final List<int> ids = built.rowIdsInOpOrder.whereType<int>().toList(
          growable: false,
        );
