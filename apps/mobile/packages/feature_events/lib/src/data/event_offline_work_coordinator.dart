...
enum OfflineWorkSyncPhase { idle, syncing, failed, needsAttention }
...
class EventOfflineWorkSnapshot {
  ...
  int get totalWorkItems =>
      checkInPending + fieldPending + chatPending + chatFailed;
}
...
class EventOfflineWorkCoordinator {
  ...
  static final EventOfflineWorkCoordinator instance =
      EventOfflineWorkCoordinator._();
  ...
  Future<void> start() async {
    ...
    tryReadRoot(authStateProvider)?.addListener(_onAuthChanged);
    scheduleDrain();
  }
  ...
  void dispose() {
    ...
    tryReadRoot(authStateProvider)?.removeListener(_onAuthChanged);
    snapshot.value = EventOfflineWorkSnapshot.empty();
    _drainChain = Future<void>.value();
  }

  Future<void> refreshSnapshot() async {
    if (!(tryReadRoot(appBootstrapProvider)?.isInitialized ?? false)) {
      snapshot.value = EventOfflineWorkSnapshot.empty();
      return;
    }
    if (!(tryReadRoot(authStateProvider)?.isAuthenticated ?? false)) {
    ...
    final int checkIn = (await CheckInSyncQueue.instance.peek()).length;
  ...
  Future<void> _runDrain() async {
    ...
    final AppBootstrap? sl = tryReadRoot(appBootstrapProvider);
    if (sl == null || !sl.isInitialized || !sl.authState.isAuthenticated) {
      await refreshSnapshot();
      return;
    }
    final List<ConnectivityResult> reachability =
    ...
    try {
      ...
      for (int i = 0; i < 50; i++) {
        final ChatOutboxEntry? entry = await ChatOutboxStore.shared
            .peekNextGlobally();
        if (entry == null) {
          break;
        }
      ...
      logEventsDiagnostic(
        'offline_work_drain_failed',
        detail: 'phase=exception',
      );
      await refreshSnapshot();
