...
EventImpactReceipt eventImpactReceiptFromJson(Map<String, dynamic> json) {
  final List<dynamic> evidenceRaw =
      safeAsList(json['evidence']) ?? const <dynamic>[];
  final List<EventImpactReceiptEvidenceItem> evidence = evidenceRaw
      .map(safeAsStringKeyedMap)
  ...
  final List<dynamic> afterRaw =
      safeAsList(json['afterImageUrls']) ?? const <dynamic>[];
  final List<String> afterUrls = afterRaw
      .map((dynamic u) => u as String)
      .toList();
  ...
  return EventImpactReceipt(
    ...
    reportedBagsCollected:
        (json['reportedBagsCollected'] as num?)?.toInt() ?? 0,
    bagsUpdatedAt: json['bagsUpdatedAt'] != null
        ? DateTime.parse(json['bagsUpdatedAt']! as String)
        : null,
    evidence: evidence,
