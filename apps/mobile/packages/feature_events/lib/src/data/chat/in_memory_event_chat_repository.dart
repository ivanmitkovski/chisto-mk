...
class InMemoryEventChatRepository implements EventChatRepository {
  final Map<String, List<EventChatMessage>> _byEvent =
      <String, List<EventChatMessage>>{};
  ...
  List<EventChatMessage> _sortedNewestFirst(String eventId) {
    final List<EventChatMessage> all = List<EventChatMessage>.from(
      _messages(eventId),
    );
  ...
  Future<EventChatFetchResult> searchMessages(
    ...
    final List<EventChatMessage> all = _sortedNewestFirst(eventId).where((
      EventChatMessage m,
    ) {
  ...
  Future<List<EventChatMessage>> fetchPinnedMessages(String eventId) async {
    return _messages(
        eventId,
      ).where((EventChatMessage m) => m.isPinned && !m.isDeleted).toList()
  ...
  Future<List<EventChatAttachment>> uploadAttachments(
    ...
    final int total = files.fold<int>(
      0,
      (int a, UploadableFile f) => a + f.bytes.length,
    );
    int sent = 0;
  ...
  Future<EventChatMessage> sendMessage(
    ...
    final List<EventChatAttachment> attach =
  ...
  Future<EventChatMarkReadResult?> markRead(
    ...
    _readCursorsByEvent.putIfAbsent(
      eventId,
      () => <String, EventChatReadCursor>{},
    )['me'] = cur;
    _bus.add(
  ...
  Future<int> fetchUnreadCount(String eventId) async {
    ...
    if (last == null || last.isEmpty) {
      return list
          .where((EventChatMessage m) => !m.isOwnMessage && !m.isDeleted)
          .length;
    }
  ...
  Future<bool> fetchMuteStatus(String eventId) async {
    return _mutedByEvent[eventId] ?? false;
  }
  ...
  Future<EventChatParticipantsResult> fetchParticipants(String eventId) async {
    ...
    return EventChatParticipantsResult(
      ...
      participants: previews.take(50).toList(),
    );
  }
  ...
  Future<List<EventChatReadCursor>> fetchReadCursors(String eventId) async {
    ...
    final Map<String, EventChatReadCursor> byUser =
        _readCursorsByEvent[eventId] ?? <String, EventChatReadCursor>{};
    return p.participants
        .map(
          (EventChatParticipantPreview x) =>
              byUser[x.id] ??
              EventChatReadCursor(userId: x.id, displayName: x.displayName),
        )
        .toList();
  }
