...
Map<String, dynamic> buildRankedEventsSearchBody({
  ...
  final Map<String, dynamic> body = <String, dynamic>{'query': q, 'limit': 30};
  ...
  if (params.categories.isNotEmpty) {
    final List<String> keys =
        params.categories.map((EcoEventCategory c) => c.key).toList()..sort();
    body['category'] = keys.join(',');
  }
  if (params.statuses.isNotEmpty) {
    final List<String> keys =
        params.statuses.map((EcoEventStatus s) => s.apiKey).toList()..sort();
