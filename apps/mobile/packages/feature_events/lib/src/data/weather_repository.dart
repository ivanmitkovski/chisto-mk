...
class WeatherRepository {
  ...
  Future<DayWeather?> _fetchFromOpenMeteoForecast({
    ...
    final Map<String, dynamic>? body = safeJsonDecodeMap(response.body);
    if (body == null || body['error'] == true) return null;

    final Map<String, dynamic>? daily = safeAsStringKeyedMap(body['daily']);
  ...
  static DayWeather? _parseDailyDayWeather(
    ...
    final List<dynamic>? dates = safeAsList(daily['time']);
    final List<dynamic>? maxTemps = safeAsList(daily['temperature_2m_max']);
    final List<dynamic>? minTemps = safeAsList(daily['temperature_2m_min']);
    final List<dynamic>? precips = safeAsList(daily['precipitation_sum']);
    final List<dynamic>? probMax = safeAsList(
      daily['precipitation_probability_max'],
    );
    final List<dynamic>? codes = safeAsList(daily['weathercode']);
  ...
  Future<DayWeather?> _readCache(String key) async {
    try {
      ...
      final Map<String, dynamic>? map = safeJsonDecodeMap(raw);
  ...
  Future<void> _writeCache(String key, DayWeather w) async {
    try {
      ...
      if (w.precipitationProbabilityMax != null) {
        payload['precipitationProbabilityMax'] = w.precipitationProbabilityMax;
      }
