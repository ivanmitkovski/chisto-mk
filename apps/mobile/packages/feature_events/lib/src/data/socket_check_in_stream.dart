...
class SocketCheckInStream {
  ...
  void connect(String eventId) {
    ...
    _socket!
      ...
      ..on('checkin:request', _onCheckInRequest)
      ..on('checkin:confirmed', _onCheckInConfirmed)
      ..on('checkin:rejected', _onCheckInRejected);
  ...
  void _onCheckInRequest(dynamic data) {
    ...
    if (pid != null &&
        ...
        ln != null &&
        exp != null) {
      if (_currentEventId != null && eid.isNotEmpty && eid != _currentEventId) {
  ...
  void _onCheckInConfirmed(dynamic data) {
    final Map<String, dynamic>? map = _asStringKeyedMap(_unwrap(data));
    if (map == null) return;
    final String? pid = map['pendingId'] as String?;
    final String? eid = map['eventId'] as String?;
  ...
  void _onCheckInRejected(dynamic data) {
    final Map<String, dynamic>? map = _asStringKeyedMap(_unwrap(data));
    if (map == null) return;
    final String? pid = map['pendingId'] as String?;
    final String? eid = map['eventId'] as String?;
  ...
  /// Simulates server socket payloads without opening a connection (unit tests).
  @visibleForTesting
  void injectSocketEventForTest(String eventName, dynamic data) {
    switch (eventName) {
      case 'checkin:request':
        _onCheckInRequest(data);
      case 'checkin:confirmed':
        _onCheckInConfirmed(data);
      case 'checkin:rejected':
        _onCheckInRejected(data);
      case 'error':
        if (data is Map && data['code'] == 'AUTH_FAILED') {
          _disconnectSocketOnly();
          _emitStatus(CheckInWsConnectionStatus.disconnected);
  ...
  /// Sets the active room for [injectSocketEventForTest] event-id filtering.
  @visibleForTesting
  void setCurrentEventIdForTest(String eventId) {
    _currentEventId = eventId;
  }
}
