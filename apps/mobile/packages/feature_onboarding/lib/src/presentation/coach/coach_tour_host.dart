...
import 'package:feature_onboarding/src/presentation/coach/coach_tour_illustrations.dart';
import 'package:feature_onboarding/src/presentation/coach/coach_tour_layout.dart';
import 'package:feature_onboarding/src/presentation/coach/coach_tour_scrim.dart';
import 'package:feature_onboarding/src/presentation/coach/coach_tour_visual_policy.dart';
import 'package:feature_onboarding/src/presentation/coach/home_shell_coach_keys.dart';
import 'package:feature_onboarding/src/presentation/widgets/story_guide_progress_bar.dart';
import 'package:flutter/material.dart';
import 'package:flutter/semantics.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:go_router/go_router.dart';

export 'package:feature_onboarding/src/presentation/coach/coach_tour_scrim.dart'
...
class CoachTourHost extends ConsumerStatefulWidget {
  const CoachTourHost({
    super.key,
    required this.keys,
  ...
  @override
  ConsumerState<CoachTourHost> createState() => _CoachTourHostState();
}

class _CoachTourHostState extends ConsumerState<CoachTourHost>
  ...
  void _onControllerChanged(CoachTourState tourState) {
    ...
    if (tourState.isCelebratingCompletion) {
      return;
    }
    _scheduleLayout();
  }
  ...
  void _scheduleLayout() {
    ...
    _measureAttempts = 0;
    _holeMeasurementFailed = false;
    final CoachTourStep step = _tour.currentStep;
  ...
  void _measureHole(int gen) {
    if (!mounted || gen != _layoutGeneration || !_tour.isVisible) {
      return;
    }
    if (_tour.isCelebratingCompletion) {
      return;
    }
    final CoachTourTarget t = _tour.currentStep.target;
    Rect? r;
    switch (t) {
  ...
  Future<void> _onCoachPrimaryPressed(
    ...
    if (_tour.isLastStep) {
      try {
        await _tour.completeWithCelebration(reduceMotion: reduceMotion);
      } catch (_) {
        if (!context.mounted) {
          return;
        }
  ...
  Widget build(BuildContext context) {
    ref.listen(coachTourControllerProvider, (
      CoachTourState? previous,
      CoachTourState next,
    ) {
      if (previous == next) return;
      _onControllerChanged(next);
    });
    final CoachTourState tour = ref.watch(coachTourControllerProvider);

    if (!tour.isVisible) {
      return const SizedBox.shrink();
    }

    if (tour.isCelebratingCompletion) {
    ...
    final int step = tour.stepIndex;
    final int total = _tour.stepCount;
    ...
    if (step != _lastAnnouncedStep) {
      ...
      // ignore: deprecated_member_use, SemanticsService.announce pending sendAnnouncement migration
    ...
    final bool stepWantsHole = _tour.currentStep.target != CoachTourTarget.none;
            ...
            child: LayoutBuilder(
              builder: (BuildContext context, BoxConstraints constraints) {
                ...
                final IconData? illustration = coachTourIllustrationIcon(
                  _tour.currentStep.illustration,
                );
...
class _CoachTooltipCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    const TextTheme tt = AppTypography.textTheme;
