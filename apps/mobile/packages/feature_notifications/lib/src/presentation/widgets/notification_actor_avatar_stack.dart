...
class NotificationActorAvatarStack extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final List<NotificationActor> visible = actors
        .take(maxVisible)
        ...
        children: <Widget>[
          for (int i = 0; i < visible.length; i++)
            Positioned(
              ...
              child: _AvatarRing(size: size, actor: visible[i]),
            ),
          if (extra > 0)
            Positioned(
      ...
      child: ClipOval(
        child: actor.avatarUrl != null
            ? CachedNetworkImage(
                ...
                errorWidget: (_, _, _) => _InitialsFallback(initials: initials),
              )
  ...
  Widget build(BuildContext context) {
    ...
    return ColoredBox(
      ...
      child: Center(
        child: Text(
          initials,
          style: AppTypography.microLabel(textTheme).copyWith(
    ...
    return Container(
      ...
      child: Text(
        ...
        style: AppTypography.badgeLabel(
          textTheme,
        ).copyWith(fontSize: size * 0.32, color: AppColors.appBackground),
      ),
    );
  }
}
