...
import 'package:chisto_infrastructure/shared/widgets/organisms/photo_gallery/photo_gallery_widgets.dart';
import 'package:design_system/design_system.dart';
import 'package:flutter/material.dart';

export 'package:chisto_infrastructure/shared/widgets/organisms/photo_gallery/photo_gallery_widgets.dart';
...
class ImmersivePhotoGallery extends StatefulWidget {
  ...
  /// use this to refresh the source so retries resolve a working URL.
...
class _ImmersivePhotoGalleryState extends State<ImmersivePhotoGallery> {
  ...
  void _prefetchInitialImages() {
    ...
    for (int i = 0; i < widget.items.length && i < 3; i++) {
      safePrecacheImage(widget.items[i].image, context);
    }
  }
              ...
              return FadeTransition(
                ...
                child: AnimatedBuilder(
                  ...
                  builder: (BuildContext context, Widget? child) {
                    final double lift =
                        (1 - curved.value) * AppSpacing.radius18;
                ...
                PageView.builder(
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    ...
                    final Widget image = AppSmartImage(
                      ...
                      decodePreset: AppSmartImageDecodePreset.auto,
                      onError: widget.onImageError,
                    );
                    final String? heroTag = item.heroTag;
                    final Widget child = heroTag != null && heroTag.isNotEmpty
                    ...
                    child: GalleryGlassPill(
                      child: Text(
                        ...
                        style: AppTypography.badgeLabel(
                          textTheme,
                        ).copyWith(fontSize: 12, color: AppColors.textOnDark),
                      ),
                    ),
                  ),
                Positioned(
  ...
  void _prefetchAround(int index, {bool shouldPrefetchCurrent = false}) {
    if (shouldPrefetchCurrent && index >= 0 && index < widget.items.length) {
      safePrecacheImage(widget.items[index].image, context);
    }
    final int previous = index - 1;
    final int next = index + 1;
    if (previous >= 0 && previous < widget.items.length) {
      safePrecacheImage(widget.items[previous].image, context);
    }
    if (next >= 0 && next < widget.items.length) {
      safePrecacheImage(widget.items[next].image, context);
    }
  }
}
...
class _FullscreenPhotoGalleryScreenState
  ...
  bool get _isCurrentImageZoomed =>
      _zoomControllers[_currentIndex].value.getMaxScaleOnAxis() >
      _kZoomedThreshold;
  ...
  void initState() {
    ...
    _dragSettleController.addListener(_onDragSettleTick);
  ...
  void _onDragSettleTick() {
    if (!mounted) return;
    setState(() {
      _verticalDrag = _verticalDragTween.evaluate(_dragSettleController);
      _backgroundOpacity = _backgroundOpacityTween
          .evaluate(_dragSettleController)
          .clamp(0.28, 1.0);
    });
  }
  ...
  void dispose() {
    ...
    _dragSettleController.removeListener(_onDragSettleTick);
  ...
  void _handleVerticalDragUpdate(DragUpdateDetails details) {
    ...
    setState(() {
      ...
      final bool crossedDismissThreshold =
          _verticalDrag.abs() > _kDismissHapticThreshold;
  ...
  void _toggleZoom(TapDownDetails details, int index) {
    ...
    final bool isZoomed =
        controller.value.getMaxScaleOnAxis() > _kZoomedThreshold;
    if (isZoomed) {
  ...
  void _handleInteractionEnd(int index) {
    ...
    _matrixSettleAnimation =
        Matrix4Tween(begin: Matrix4.copy(current), end: target).animate(
          CurvedAnimation(
                    ...
                    child: Stack(
                      children: <Widget>[
                        Positioned(
                          ...
                          child: Semantics(
                            button: true,
                            label: context.l10n.semanticsClose,
                            child: GestureDetector(
                              onTap: () {
                                Navigator.of(context).pop();
                              },
