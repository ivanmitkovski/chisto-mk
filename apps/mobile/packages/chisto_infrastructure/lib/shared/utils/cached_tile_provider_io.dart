...
class _CachedTileProvider extends TileProvider {
  ...
  Future<String> _resolveCachePath() async {
    final String dir = (await getTemporaryDirectory()).path.replaceAll(
      '/',
...
class _CachedTileImageProvider extends ImageProvider<_CachedTileImageProvider> {
  ...
  Future<ui.Codec> _loadAsync(ImageDecoderCallback decode) async {
    ...
    if (file.existsSync()) {
      final DateTime mod = file.lastModifiedSync();
      final bool stale = DateTime.now().difference(mod).inDays > maxStaleDays;
      if (!stale) {
        try {
