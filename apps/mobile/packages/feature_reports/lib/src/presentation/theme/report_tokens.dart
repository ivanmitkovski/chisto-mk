...
class ReportTokens {
  ...
  static const double photoGridAspectRatio = 1;
  ...
  /// Fixed width of the label column in [ReportDetailRow]. A constant width keeps
  /// every value left-aligned to the same x for a balanced, scannable layout
  /// (labels wrap within this width at large text scales).
  static const double detailRowLabelWidth = 112;
  ...
  static const Duration locationGeocodeDebounceMap = Duration(
    milliseconds: 700,
  );

  /// One-shot geocode after GPS fix (snappier feedback).
  static const Duration locationGeocodeDebounceGps = Duration(
    milliseconds: 400,
  );

  /// Auto-confirm pin after user stops panning (see [LocationPickerController]).
  static const Duration locationAutoConfirmStable = Duration(
    milliseconds: 1500,
  );
}
