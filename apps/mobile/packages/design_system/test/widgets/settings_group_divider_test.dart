...
void main() {
  testWidgets(
    'SettingsGroupDivider renders a single full-width hairline segment',
    (WidgetTester tester) async {
      ...
      final RenderBox rowBox = tester.renderObject<RenderBox>(
        find.descendant(of: dividerRow, matching: find.byType(Row)),
      );
      final RenderBox hairlineBox = tester.renderObject<RenderBox>(
        find.descendant(of: dividerRow, matching: find.byType(ColoredBox)),
      );

      expect(
        hairlineBox.size.width,
        rowBox.size.width - SettingsGroupDivider.leadingInset,
      );
  ...
  testWidgets(
    'SettingsListTile omits divider on last row when showDividerBelow is false',
    (WidgetTester tester) async {
      await tester.pumpWidget(
        MaterialApp(
          theme: AppTheme.light,
