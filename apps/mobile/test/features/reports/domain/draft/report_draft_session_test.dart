import 'package:feature_reports/src/domain/draft/report_draft_session.dart';
import 'package:feature_reports/src/domain/draft/report_stage.dart';
...
void main() {
  group('reduceReportDraftSession', () {
    test('SessionDraftEdited updates draft', () {
      ...
      final ({
        ReportDraftSessionState state,
        List<ReportDraftSessionEffect> effects,
      })
      r = reduceReportDraftSession(state: s0, event: SessionDraftEdited(next));
    ...
    test('SessionStageAttempted accumulates stages', () {
      ...
      expect(
        s.attemptedStages,
        containsAll(<ReportStage>[ReportStage.details, ReportStage.location]),
      );
    });

    test(
      'SessionSubmitStarted sets submitting and marks all stages attempted',
      () {
        final ReportDraftSessionState s0 = ReportDraftSessionState.initial();
        final ({
          ReportDraftSessionState state,
        ...
        r = reduceReportDraftSession(state: s0, event: SessionSubmitStarted());
    ...
    test('SessionSubmitEnded clears submitting', () {
      ...
      r = reduceReportDraftSession(state: s0, event: SessionSubmitEnded());
    ...
    test('SessionCleared resets to initial', () {
      ...
      r = reduceReportDraftSession(state: s0, event: SessionCleared());
    ...
    test('SessionAutosaveFired emits persist when body is persistable', () {
      final ReportDraftSessionState s0 = ReportDraftSessionState.initial()
          .copyWith(draft: ReportDraft(photos: <XFile>[XFile('p.jpg')]));
      final ({
        ReportDraftSessionState state,
        List<ReportDraftSessionEffect> effects,
      })
      ...
      final EffectPersistWizardDraft e =
          r.effects.single as EffectPersistWizardDraft;
      expect(e.titleText, '');
      expect(e.descriptionText, '');
    });

    test(
      'SessionAutosaveFired emits persist when title or description non-empty',
      () {
        final ReportDraftSessionState s0 = ReportDraftSessionState.initial();
        final ({
          ReportDraftSessionState state,
    ...
    test(
      'SessionAutosaveFired persists when past evidence even with empty texts',
      () {
        final ReportDraftSessionState s0 = ReportDraftSessionState.initial()
            .copyWith(currentStage: ReportStage.details);
        final ({
          ReportDraftSessionState state,
          List<ReportDraftSessionEffect> effects,
        })
    ...
    test('SessionAutosaveFired no effect when submitting', () {
      final ReportDraftSessionState s0 = ReportDraftSessionState.initial()
          .copyWith(draft: ReportDraft(title: 'A'), submitting: true);
      final ({
        ReportDraftSessionState state,
        List<ReportDraftSessionEffect> effects,
      })
  ...
  group('ReportDraftSessionState.shouldPersistWizardDraft', () {
    test(
      'true when attemptedStages non-empty even on evidence with empty texts',
      () {
        final ReportDraftSessionState s = ReportDraftSessionState.initial()
            .copyWith(attemptedStages: <ReportStage>{ReportStage.details});
        expect(
          s.shouldPersistWizardDraft(titleText: '', descriptionText: ''),
          isTrue,
        );
      },
    );
