...
void main() {
  group('ecoEventFromJson', () {
    ...
    test('tolerates bad endAt by falling back to start + 2h', () {
      ...
      expect(event!.endTime.totalMinutes - event.startTime.totalMinutes, 120);
    });
    ...
    test('maps scheduledAt/endAt into date and time fields', () {
      ...
      expect(event.endTime.totalMinutes > event.startTime.totalMinutes, isTrue);
    ...
    test('attendeeCheckedInAt from API is local after ecoEventFromJson', () {
      ...
      expect(
        event.attendeeCheckedInAt!.isUtc,
        isFalse,
    ...
    test(
      'maps moderation and check-in flags used by detail + check-in flows',
      () {
        final EcoEvent? event = ecoEventFromJson(<String, dynamic>{
          'id': 'evt-mod',
          'title': 'T',
  ...
  group('parsePointsAwardedFromJson', () {
    test('reads int and num', () {
      expect(
        parsePointsAwardedFromJson(<String, dynamic>{'pointsAwarded': 12}),
        12,
      );
      expect(
        parsePointsAwardedFromJson(<String, dynamic>{'pointsAwarded': 15.0}),
        15,
      );
    });
