...
void main() {
  group('CheckInQrPayload', () {
    ...
    test(
      'parses v2 token envelope for UI (signature not verified client-side)',
      () {
        ...
        final String body = base64Url.encode(utf8.encode(json.encode(claims)));
    ...
    test('fromOrganizerQrApiJson parses expiresAt and session', () {
      ...
      final CheckInQrPayload? payload =
          CheckInQrPayload.fromOrganizerQrApiJson(<String, dynamic>{
            'qrPayload': raw,
            'sessionId': 'sess-row',
      ...
      expect(payload.expiresAtMs, expiresUtc.toLocal().millisecondsSinceEpoch);
    });

    test(
      'fromOrganizerQrApiJson returns null when expiresAt missing is ok',
      () {
        final Map<String, dynamic> claims = <String, dynamic>{
          'e': 'evt-x',
          's': 's',
