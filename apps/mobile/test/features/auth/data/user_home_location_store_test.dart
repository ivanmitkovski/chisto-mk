...
void main() {
  group('UserHomeLocationStore', () {
    ...
    test('applyFromProfileJson clears when homeLocationSetAt is null', () async {
    ...
    test('applyFromProfileJson saves confirmed home with scoped keys', () async {
      final UserHomeLocationStore store = UserHomeLocationStore(
        prefs,
        userId: 'user-1',
      );
    ...
    test('scoped store ignores legacy global coords until server sync', () async {
      await prefs.setDouble(kUserHomeLatitudeKey, 41.99);
      await prefs.setDouble(kUserHomeLongitudeKey, 21.43);
