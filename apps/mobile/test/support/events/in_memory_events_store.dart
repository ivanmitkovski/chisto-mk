...
import '../events/mock_eco_events.dart';
...
class InMemoryEventsStore extends ChangeNotifier implements EventsRepository {
  ...
  @override
  Future<void> get ready => _hydrationCompleter?.future ?? Future<void>.value();
  ...
  Future<EventsListPageSnapshot> fetchEventsFilterPreview(
    ...
    final List<EcoEvent> events = await fetchEventsSnapshot(params);
  ...
  /// Widget tests: duplicate the first seeded event under [id] (e.g. push UUID payloads).
  @visibleForTesting
  void seedAliasEventId(String id) {
    final EcoEvent template = _events.first;
    _events = <EcoEvent>[
      EcoEvent(
        id: id,
        ...
        siteImageUrl: template.siteImageUrl,
        siteDistanceKm: template.siteDistanceKm,
        organizerId: template.organizerId,
        organizerName: template.organizerName,
        organizerAvatarUrl: template.organizerAvatarUrl,
        date: template.date,
        ...
        isJoined: template.isJoined,
        scheduledAtUtc: template.scheduledAtUtc,
        moderationApproved: template.moderationApproved,
      ),
      ..._events,
    ];
    notifyListeners();
  ...
  }) async => const EventScheduleConflictPreview(hasConflict: false);
  ...
  Future<EcoEvent> updateEventDetails(
    ...
    if (patch.containsKey('gear')) {
      ...
      final List<EventGear> gear = raw
          .map(
            (dynamic g) => EventGear.values.firstWhere(
  ...
  Future<bool> setReminder({
    ...
    _events = _events
        .map((EcoEvent event) {
          ...
          if (event.reminderEnabled == enabled &&
  ...
  Future<EventParticipantsPage> fetchParticipants(
    ...
    if (c != null && c.isNotEmpty) {
      return const EventParticipantsPage(
